...
def buttons(master, var, merge, qmerge, qcurr, qnext=None):
	...
	if not merge:
		...
		b1.grid(row = 2, column = 0, pady=5, padx=5, rowspan=2)
		...
		b3.grid(row = 5, column = 0, pady=5, padx=5)

		b4 = Button(master, text="Skip\nEnd Merge", command=fn(4, master, var))
		b4.grid(row = 6, column = 0, pady=5, padx=5, rowspan=2)
	...
	if not (qnext == None): 
	       ...
	       nextLabel.grid(row = 3, column = 1, columnspan = 1, rowspan = 1, padx = 5, pady = 5)

	       nextPic = Label(master, image = qnext)
	       nextPic.grid(row = 4, column = 1, columnspan = 1, rowspan = 2, padx = 5, pady = 5)

	if merge:
		mergeLabel = Label(master, text="Current Merge")
		mergeLabel.grid(row = 6, column = 1, columnspan = 1, rowspan = 1, padx = 5, pady = 5)

		mergePic = Label(master, image = qmerge)
		mergePic.grid(row = 7, column = 1, columnspan = 1, rowspan = 2, padx = 5, pady = 5)
